...
class ATARI:
    ...
    def cache(self, state, next_state, action, reward, done):
        ...
        """
        state = torch.tensor(state, dtype=torch.bool).cuda() if self.use_cuda else torch.FloatTensor(state)
        next_state = torch.tensor(next_state, dtype=torch.bool).cuda() if self.use_cuda else torch.FloatTensor(next_state)
